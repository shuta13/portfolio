<template lang="pug">
  .container
    #canvas(ref="canvas")
    .letters-wrap
      span.letters This is your own choice.
</template>
...
export default {
  ...
  methods: {
    ...
    createObjects () {
      ...
      for (let i = 0; i < 30; i++) {
        if (i % 2 === 0) { this.color = red } else if (i % 3 === 0) { this.color = green } else { this.color = blue }
        ...
        if (this.objectX <= 20) { this.objectX += 15 } else {
          this.objectX = -30
          this.objectY += 8
        }
      }
